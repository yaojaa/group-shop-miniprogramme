...
Page({
  ...
  toHelpSetting() {
    if (
      ...
      this.data.spec[0].spec_name == '' ||
      this.data.spec[0].spec_price == ''
    ) {
      return wx.showToast({
        title: '请先填写完整',
        icon: 'none'
  ...
  specDescFocus(e) {
    ...
    if (
      this.data.spec[index].spec_pic.length ||
      this.data.spec[index].spec_desc.length > 20
    ) {
